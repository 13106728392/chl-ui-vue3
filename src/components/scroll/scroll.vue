<template>
  <div class="c-scroll" :style="{ height: `${viewHeight}px` }" @mouseenter="mouseover" @mouseleave="mouseout">
    <div class="c-scroll-content" :style="{ paddingRight: `${size}px` }" @scroll="viewScroll">
      <slot></slot>
    </div>
    <transition name="fade">
      <div class="c-scroll-bar" v-show="!alwaysVisible || isShow" :style="{ width: `${size}px` }"
        @mousedown="thumbDrag($event)">
        <div class="c-scroll-thumb" ref="thumb" :style="{
          height: `${BarHeight}px`,
          top: `${BarTop}px`,
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const thumbDrag = (e) => {
      ...
      if (element.className === 'c-scroll-bar') {
