<template>
  <Teleport to="body" :disabled="!appendToBody">
    <transition name="fade-pupop">
      <div class="c-drawer-pupop" v-if="modelValue" @click="handleWrapperClick"></div>
    </transition>
    <transition 
      :name="animateName"
      ...
      <div 
        ...
        :style="drawerStyle"
      >
        <div class="c-drawer" :class="customClass">
          <div class="c-drawer__header" v-if="withHeader">
            <slot name="header">
              <span class="c-drawer__title">{{ title }}</span>
            </slot>
            <i class="icon c-icon-x" @click="handleClose" v-if="showClose"></i>
          </div>
...
const handleWrapperClick = (e) => {
  if(props.wrapperClosable){
    if(e.target.className=='c-drawer-pupop'){
      handleClose()
    }
  }
}
